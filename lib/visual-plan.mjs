...
export function buildLocalVisualPlan({script,universe,character,characterBible}){
  ...
  const bibleAnchor=clean(characterBible?.identityAnchor);
  const bibleRules=Array.isArray(characterBible?.continuityRules)?characterBible.continuityRules.map(clean).filter(Boolean):[];
  const continuityBase=bibleAnchor?`Conserver l’identité canonique de ${clean(character)||'du personnage'} : ${bibleAnchor}`:`Conserver le personnage ${clean(character)||'du projet'}`;
  const shots=narration.map((excerpt,index)=>{
    ...
    return {
      ...
      continuity:`${continuityBase}. Respecter l’univers ${clean(universe)||'du projet'} et les invariants canoniques sur tous les plans.`,
      continuityRules:bibleRules,
  ...
  return {
    ...
    model:'local-visual-plan-v2',
    limitations:'Plan visuel structurel dérivé sans IA externe. Les passages et faits source sont traçables et la continuité renvoie à une bible personnage validée ; les références documentaires et les choix d’illustration doivent encore être vérifiés avant génération d’images.'
  };
}
